...
Alleup.prototype = {
	...
	upload: function(req, res, callback, _scope) {
        ...
        var _this = this;
        var new_file = req.files.upload;

        _this.makeVariants(new_file, function(err, file,new_file){
		callback(err, file,new_file, res);
        });
					...
					for(prefix in _crop) {
						_this.imAction('im.crop', file, _this.genFileName( new_file, prefix ) , _crop[prefix], function(err){
							...
							if (i == Object.keys(_crop).length ) {
							    fs.unlink(file['path']);
                                console.log("*****************in make variants********************************");
                                console.log(file);
                                console.log("********************************end in make variants********************************");
							    callback(err, file,new_file);
							};
						});
					};
				};
